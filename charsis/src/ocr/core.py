"""
OCR核心模块 - 对segment生成的字符图片进行OCR识别和重命名
"""
import os
import sys
import re
...
def recognize_character_images(input_dir: str, output_dir: str, config: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    对文件夹中的字符图片进行OCR识别和重命名
    
    :param input_dir: 输入目录（segment生成的字符图片文件夹）
    :param output_dir: 输出目录（OCR处理后的文件夹）
    :param config: OCR配置
    :return: 处理统计信息
    """
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')) and filename.startswith('char_'):
    ...
    for i, filename in enumerate(char_files, 1):
        ...
        try:
            ...
            if not recognized_text:
                output_filename = f"empty_{i:03d}.png"
                stats['empty'] += 1
                ocr_result['status'] = 'empty'
            elif confidence < config.get('confidence_threshold', 0.3):
                output_filename = f"low_conf_{safe_filename(recognized_text)}_{i:03d}.png"
                stats['low_confidence'] += 1
                ocr_result['status'] = 'low_confidence'
            else:
                # 使用识别的文字作为文件名
                safe_text = safe_filename(recognized_text)
                name_counter[safe_text] += 1
                
                if name_counter[safe_text] == 1:
                    output_filename = f"{safe_text}.png"
                else:
                    output_filename = f"{safe_text}_{name_counter[safe_text]:02d}.png"
    ...
    print(f"成功识别: {stats['recognized']}")
    print(f"空识别: {stats['empty']}")
...
def process_all_segment_results(input_base_dir: str = None, output_base_dir: str = None, 
                               config: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    批量处理所有segment结果
    
    :param input_base_dir: segment结果基础目录，默认为SEGMENTS_DIR
    :param output_base_dir: OCR结果基础目录，默认为OCR_DIR  
    :param config: OCR配置
    :return: 批量处理统计信息
    """
    if input_base_dir is None:
        input_base_dir = SEGMENTS_DIR
    if output_base_dir is None:
        output_base_dir = OCR_DIR
    ...
    for item in os.listdir(input_base_dir):
        ...
        if os.path.isdir(item_path):
            # 检查是否包含字符图片
            has_char_files = any(f.startswith('char_') and f.lower().endswith(('.png', '.jpg', '.jpeg'))
    ...
    print(f"找到 {len(image_dirs)} 个图片文件夹")
    
    batch_stats = {
        'total_dirs': len(image_dirs),
        'processed_dirs': 0,
        'total_files': 0,
        'total_recognized': 0,
        'total_empty': 0,
    ...
    for i, dir_name in enumerate(image_dirs, 1):
        print(f"\n{'='*60}")
        print(f"处理进度: {i}/{len(image_dirs)} - {dir_name}")
        print(f"{'='*60}")
        
        input_dir = os.path.join(input_base_dir, dir_name)
    ...
    print(f"成功识别: {batch_stats['total_recognized']}")
    print(f"空识别: {batch_stats['total_empty']}")
    print(f"低置信度: {batch_stats['total_low_confidence']}")
    print(f"总错误: {batch_stats['total_errors']}")
    
    if batch_stats['total_files'] > 0:
        success_rate = batch_stats['total_recognized'] / batch_stats['total_files'] * 100
...
if __name__ == '__main__':
    ...
    if batch_stats.get('total_dirs', 0) == 0:
        print("\n没有找到可处理的segment结果。")
        print("请先运行segment模块生成字符图片，然后再运行OCR处理。")
